...
for i = 1:1:size(fams,2)
    monTable{size(fams{i},2),1}{hashFun(fams{i},maxHash)}{end+1} = fams{i};
end
clearvars fams
               ...
               for cei = 2:1:maxSize-1
                    for cerh = 1:1:maxHash
                        for cerhi = 1:1:size(monTable{cei,end-1}{cerh},2)
                            ...
                            if(prodsize <= maxSize)
                                hash = hashFun(product,maxHash);
                                [logic,monTable{prodsize,end}{hash}] = bubbleDiff(monTable{prodsize,end}{hash}, product);
                                if logic == 1
                                    dcLoop = 0;
                                end
                            end
    ...
    for i = 1:1:maxSize
        for j = 1:1:size(monTable,2)-1
            for k = 1:1:size(monTable{i,j},2)
                monTable{i,end}{k} = slowPruneCell(monTable{i,j}{k},monTable{i,end}{k});
            end
        end
    end
end
