...
while(~dcLoop)
    dcLoop = 1;
    %Make new column
    monTable(1:end,end+1) = cell(numMaps,1);
    for i = 2:1:numMaps
        for j = 1:1:size(monTable,2)-1
            for k = 1:1:size(monTable{i,j},2)
               for m = 1:1:numMaps
                   for n = 1:1:size(monTable{m,end-1},2)
                        product = closure(monTable{i,j}{1,k},monTable{m,end-1}{n},id,compTable);
                        prodsize = size(product,2);
                        if(prodsize <= maxSize)
                            [logic,monTable{prodsize,end}] = inclusionDiff(monTable{prodsize,end}, product);
                            if logic == 1
                                dcLoop = 0;
                            end
                        end
    ...
    for i = 1:1:numMaps
        for j = 1:1:size(monTable,2)-1
            for k = 1:1:size(monTable{i,j},2)
                for m = size(monTable{i,end},2):-1:1
                    if(isequal(monTable{i,j}{k},monTable{i,end}{m}))
                        monTable{i,end} = monTable{i,end}([1:m-1 m+1:end]);
                    end
                end
            end
        end
...
monTable = monTable(1:end,1:end-2);
clearvars dcLoop logic product prodsize numMaps compTable combos k m n id
